...
3-bullet agenda lifted from Pass 14 deferred items + Bucket C page-hygiene work:

1. **axe-core baseline apply** (Bucket B #10, T1): requires `bash scripts/axe-core.sh` from a creds-loaded TTY + locally-installed Chrome (per Pass 13 / 13.7 deferral). Operator runs once after `bin/prep-push.sh` lands; emits `docs/agents/a11y-baseline-2026-07-XX.md`. Token cost ~25k.
2. **`scripts/page-budget.sh` audit** (Bucket C #18, T2, ~25k tokens): run on Pages collection; warn if any page < 6 sections; emit `docs/agents/page-budget-2026-07-XX.md`. Score +0.
3. **Contact-block field rewire** (Bucket C #16, T1): replace the `[phone-add-in-tina-admin]` placeholder on `contact.mdx` + `faq.mdx` with real Tina fields from `tina/collections/global-config.ts`; remove the placeholder-handling intent. Score +0.
