...
- **Last updated**: pass 19 - 2026-07-09 (ZakatCalculator integrated on /donate; Callout shape consistency; em-dash sweep across all .astro files; design-taste-frontend eyebrow restraint applied; GitHub issue #52 created)
...
1 / 2 / 3 / 4 / 5 / 6 / 7 / 8 / 8.1 / 8.2 / 8.3 / 9 / 9.1 / 9.2 / 10 / 11 / 12 / 12.1 / 12.2 / 13 / 13.1 / 13.2 / 13.4 / 13.5 / 13.6 / 13.7 / 13.8 / 13.9 / 13.8 / 13.9 / 14 / 14.1 / 14.2 / 15 / 16 / 16.1 / 16.2 / 17 / 18 / 19
...
3-bullet agenda lifted from Pass 15 deferred items + the running carry-over from Pass 11-14:

1. **axe-core baseline apply** (Bucket B #10, T1): requires `bash scripts/axe-core.sh` from a creds-loaded TTY + locally-installed Chrome (per Pass 13 / 13.7 deferral). Operator runs once after `bin/prep-push.sh` lands; emits `docs/agents/a11y-baseline-2026-07-XX.md`. Token cost ~25k. With Pass 15's content burst the surface to audit grew; one extra page-budget per new content type recommended.
2. **Contact-block field rewire** (Bucket C #16, T1): replace the `[phone-add-in-tina-admin]` placeholder on `contact.mdx` + `faq.mdx` with real Tina fields from `tina/collections/global-config.ts`; remove the placeholder-handling intent. Score +0.
3. **Re-read the new blog + event surface for SEO + index** (Pass 15 carry-over, T2): confirm `/blog` index renders all 21 posts and `/events` renders all 15 events with hero images; check `rss.xml.ts` includes the new events; verify the [search index](/blog/search.json.ts) is regenerated. Score +0.
